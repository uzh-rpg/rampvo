...
class EventSequence(object):
    def __init__(self, dataframe, params, features=None, timestamp_multiplier=None, convert_to_relative=False):
        if isinstance(dataframe, pandas.DataFrame):
            ...
            self.num_events = len(dataframe.index)
        else:
            ...
            self.num_events = len(self.features[-1])
...
def get_events_from_npz(event_path):
    # It's possible that there is no event file! (camera standing still)
    try:
        with numpy.load(event_path) as f:
            event_np = numpy.stack([f["t"],f["x"],f["y"],f["p"]],axis=1)
            return pd.DataFrame(event_np, columns=['ts','x','y','p'])
